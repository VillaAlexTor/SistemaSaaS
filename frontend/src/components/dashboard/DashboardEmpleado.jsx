...
function DashboardEmpleado({ usuario, cerrarSesion }) {
  ...
  const cargarDatos = async () => {
    setCargando(true);
    console.log('📊 Cargando datos del empleado:', usuario);

    try {
      // Cargar datos de la microempresa del empleado
      const [resProductos, resVentas, resClientes] = await Promise.all([
        api.getProductosMicroempresa(usuario.microempresa_id),
        api.getVentasMicroempresa(usuario.microempresa_id),
        api.getClientesCompradores(usuario.microempresa_id)
      ]);
      ...
      <div style={{
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            ...
            <div>
              <h1 style={{ margin: 0, color: '#4caf50', fontSize: '24px' }}>
                Panel de Empleado
              </h1>
              <p style={{ margin: 0, color: '#aaa', fontSize: '13px' }}>
                {usuario.nombre} • {usuario.microempresa_nombre}
              </p>
              <p style={{ margin: 0, color: '#666', fontSize: '11px' }}>
                Plan: <span style={{ color: usuario.microempresa_plan === 'premium' ? '#ff9800' : '#666', fontWeight: 'bold' }}>
                  {usuario.microempresa_plan?.toUpperCase() || 'BÁSICO'}
                </span>
              </p>
            </div>
          </div>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '30px' }}>
        ...
        {cargando ? (
          <div style={{ textAlign: 'center', padding: '60px', backgroundColor: '#2d2d2d', borderRadius: '10px', border: '1px solid #3d3d3d' }}>
            ...
            <p style={{ color: '#4caf50', fontSize: '18px', margin: 0 }}>Cargando tu panel...</p>
          </div>
        ) : (
          <>
            ...
            <div style={{
              ...
              <div style={{ flex: 1 }}>
                <h2 style={{ margin: 0, color: '#4caf50', fontSize: '20px' }}>
                  ¡Bienvenido, {usuario.nombre}!
                </h2>
                <p style={{ margin: '5px 0 0 0', color: '#aaa', fontSize: '14px' }}>
                  Este es tu panel de trabajo. Aquí podrás gestionar productos, registrar ventas y ver clientes de {usuario.microempresa_nombre}.
                </p>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '30px' }}>
              ...
              <div style={{
                ...
                <h3 style={{ margin: '0 0 20px 0', color: '#4caf50', borderBottom: '2px solid #4caf50', paddingBottom: '10px' }}>
                  📦 Productos en Stock
                </h3>
            ...
            <div style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                ...
                <ActionButton icono="📦" texto="Ver Productos" color="#2196f3" />
                <ActionButton icono="👥" texto="Ver Clientes" color="#ff9800" />
                <ActionButton icono="📊" texto="Mis Estadísticas" color="#9c27b0" />
                <ActionButton icono="⚙️" texto="Mi Perfil" color="#666" />
              </div>
            </div>
              ...
              <div style={{
                ...
                <div>
                  ...
                  <p style={{ margin: '5px 0 0 0', color: '#aaa', fontSize: '13px' }}>
                    Hay {estadisticas.productosStockBajo} producto(s) con stock bajo. Notifica al encargado para reabastecer.
                  </p>
                </div>
              </div>
            )}
